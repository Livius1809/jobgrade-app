...
export default function MasterSimulatorLayout({ data, isDemo = false, masterContent, simulatorContent }: Props) {
  ...
  return (
    <SimulatorContext.Provider value={contextValue}>
      <div className={`transition-all duration-300 ${isSimulatorOpen ? "flex justify-center gap-8 px-8" : ""}`}>
        {/* Master — stânga */}
        <div className={`transition-all duration-300 overflow-x-hidden ${isSimulatorOpen ? "max-w-[1000px] flex-1 min-w-0" : "w-full max-w-6xl mx-auto"}`}>
          {masterContent}
        </div>

        {/* Simulator — dreapta */}
        {isSimulatorOpen && (
          <div className="w-[380px] shrink-0 sticky top-16 h-[calc(100vh-5rem)] overflow-y-auto">
