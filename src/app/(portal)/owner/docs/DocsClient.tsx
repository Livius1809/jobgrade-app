...
export default function DocsClient({ ingestKey }: { ingestKey: string }) {
  ...
  async function submitUpload() {
    ...
    try {
      ...
      if (ext === "pdf") {
        ...
        const startRes = await fetch("/api/v1/kb/ingest-chunked", { method: "POST", headers: { "x-internal-key": ingestKey }, body: fd })
      ...
      setMessage(`${textChunks.length} secțiuni pregătite. Se trimite...`)
  ...
  async function submitBibliography() {
    ...
    try {
      if (bibFile) {
        ...
        const res = await fetch("/api/v1/kb/ingest", { method: "POST", headers: { "x-internal-key": ingestKey }, body: fd })
        const data = await res.json()
