...
const AGENT_ROLE = "CARD4_COACH"
const THREAD_TYPE = "B2C_VALUES" as const
...
function loadSystemPrompt(): string {
  ...
  try {
    const filepath = join(process.cwd(), "src", "lib", "agents", "system-prompts", "card-4-system-prompt.md")
...
const SAFETY_TRIGGERS = [
  ...
  { pattern: /nu\s*mai\s*are\s*sens|nimic\s*nu\s*conteaz[aă]|sunt\s*un\s*e[sș]ec/i, level: "HIGH" as const },
]
...
// ── POST /api/v1/b2c/card-4/chat ─────────────────────────────────────────

/**
 * Chat cu Coach-ul de Dezvoltare Personala B2C (Card 4 — "Oameni de succes/valoare").
 * Tranzitia orgoliu→smerenie, Hawkins <200→>200, virtuti cultivate, sfera binelui.
 *
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 1. Verifica user B2C + Card 4 activ
    ...
    const card4 = user.cards.find((c: any) => c.card === "CARD_4")
    if (!card4 || card4.status === "LOCKED") {
      return NextResponse.json(
        { error: "Card 4 nu este activat. Activeaza-l pentru a vorbi cu Coach-ul de Dezvoltare Personala." },
        { status: 403 }
      )
    }
    ...
    if (safety.level === "CRITICAL") {
      await p.b2CEvolutionEntry.create({
        data: {
          userId, card: "CARD_4", type: "MILESTONE",
          title: "Escaladare SAFETY_MONITOR — nivel CRITIC",
          phase: card4.phase, stage: card4.stage, agentRole: AGENT_ROLE,
        },
      }).catch(() => {})
    ...
    // 8. System prompt — Coach Dezvoltare Personala: sobru, calm, provocator
    ...
    const fullSystemPrompt = [
      ...
      `Alias: ${user.alias}`,
      `Card 4: faza ${card4.phase}, etapa ${card4.stage}`,
      profileLines,
      evolutionContext,
      ...
      "",
      "--- CALIBRARE COACH DEZVOLTARE PERSONALA ---",
      "Raspunsuri de 2-4 paragrafe. Sobre, calme, mature.",
      "Nu moralizezi, nu predai, nu impui.",
      "Intrebari care provoaca reflectie profunda.",
      "ZERO jargon spiritual sau filozofic. Limbaj simplu, direct, uman.",
      "Nu mentionezi NICIODATA: Hawkins, Herrmann, VIA, Maslow, nivel de constiinta, scala, ego.",
      `Mesaj #${messageCount + 1} din conversatie — ${messageCount === 0 ? "primul contact pe Card 4" : "continuare drum"}.`,
      safety.level === "HIGH"
    ...
    await p.conversationThread.update({
      where: { id: thread.id },
      data: {
        title: thread.messages.length === 0 ? `Coach Valoare — ${user.alias}` : undefined,
        updatedAt: new Date(),
      },
    })
    ...
    try {
      ...
      await learningFunnel({
        ...
        success: true, metadata: { source: "card-4-chat", threadId: thread.id },
      })
    } catch {}
