...
const AGENT_ROLE = "CARD5_COACH"
const THREAD_TYPE = "B2C_ENTREPRENEURSHIP" as const
...
function loadSystemPrompt(): string {
  ...
  try {
    const filepath = join(process.cwd(), "src", "lib", "agents", "system-prompts", "card-5-system-prompt.md")
...
const SAFETY_TRIGGERS = [
  ...
  { pattern: /am\s*pierdut\s*tot|sunt\s*ruinat|faliment/i, level: "HIGH" as const },
]
...
// ── POST /api/v1/b2c/card-5/chat ─────────────────────────────────────────

/**
 * Chat cu Coach-ul Antreprenorial B2C (Card 5 — "Antreprenoriat transformational").
 * 2 forme (capital+munca SAU caracter+virtuti), integrare totala, undita nu pestele.
 * Poarta catre Business #2.
 *
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 1. Verifica user B2C + Card 5 activ
    ...
    const card5 = user.cards.find((c: any) => c.card === "CARD_5")
    if (!card5 || card5.status === "LOCKED") {
      return NextResponse.json(
        { error: "Card 5 nu este activat. Activeaza-l pentru a vorbi cu Coach-ul Antreprenorial." },
        { status: 403 }
      )
    }
    ...
    if (safety.level === "CRITICAL") {
      await p.b2CEvolutionEntry.create({
        data: {
          userId, card: "CARD_5", type: "MILESTONE",
          title: "Escaladare SAFETY_MONITOR — nivel CRITIC",
          phase: card5.phase, stage: card5.stage, agentRole: AGENT_ROLE,
        },
      }).catch(() => {})
    ...
    // 8. System prompt — Coach Antreprenorial: pragmatic, energizant, realist
    ...
    const fullSystemPrompt = [
      ...
      `Alias: ${user.alias}`,
      `Card 5: faza ${card5.phase}, etapa ${card5.stage}`,
      profileLines,
      evolutionContext,
      ...
      "",
      "--- CALIBRARE COACH ANTREPRENORIAL ---",
      "Raspunsuri de 2-4 paragrafe. Pragmatice, orientate spre actiune.",
      "Oferi undita, nu pestele. NU faci planuri de afaceri.",
      "Alternezi intre reflectie si actiune. Ancorezi in realitate.",
      "ZERO jargon de startup sau MBA. Limbaj direct, uman.",
      "Nu mentionezi NICIODATA: Hawkins, Herrmann, VIA, Business #2, motor fractal, engine, pivot, disruptiv.",
      `Mesaj #${messageCount + 1} din conversatie — ${messageCount === 0 ? "primul contact pe Card 5" : "continuare drum"}.`,
      safety.level === "HIGH"
        ? "ATENTIE: semnale subclinice detectate (burnout, criza financiara). Monitorizeaza cu grija."
        : "",
    ].filter(Boolean).join("\n")
    ...
    await p.conversationThread.update({
      where: { id: thread.id },
      data: {
        title: thread.messages.length === 0 ? `Coach Antreprenorial — ${user.alias}` : undefined,
        updatedAt: new Date(),
      },
    })
    ...
    try {
      ...
      await learningFunnel({
        ...
        success: true, metadata: { source: "card-5-chat", threadId: thread.id },
      })
    } catch {}
